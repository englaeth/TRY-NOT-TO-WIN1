...
function loadAssets(callback) {
  backgroundImage.src = 'https://cdn.leonardo.ai/users/0fdb9133-c970-4b73-9dba-9229c55cf8aa/generations/7970e940-b373-4175-95e2-19ecb615132d/Default_platformer_game_background_high_quality_0.jpg';
  backgroundImage.onload = function () {
    characterImage1.src = 'https://i.imgur.com/rOcjbPE.png';
    characterImage1.onload = function () {
      characterImage2.src = 'https://i.imgur.com/rOcjbPE.png';
...
const player = {
  x: 50,
  y: canvas.height - 150 - 50, // Subtract the player's height
...
function detectPlatformCollision() {
  ...
  function render() {
    ...
    // Draw the score box, high score box, and top middle box
    drawScoreBox();
    drawHighScoreBox();
    drawTopMiddleBox(); // Add this line
...
function drawScoreBox() {
  const text = `النتيجة: ${player.score}`;
  ...
  // Draw the text
  ctx.fillText(text, textX, boxY + 30);
}

  
  function drawHighScoreBox() {
  const text = `اعلى نتيجة : ${highScore}`;
  const textWidth = ctx.measureText(text).width;

  const boxWidth = textWidth + 20; // Add some padding
  const boxX = canvas.width - boxWidth - 5;
  const boxY = 5;
  ...
  // Draw the text
  ctx.fillText(text, boxX + 10, boxY + 30);
}
...
function drawTopMiddleBox() {
  ...
  function drawGameOverAndReset() {
    if (gameOver) {
   ctx.fillStyle = 'white';
ctx.font = 'bold 40px Arial';  // Add 'bold' before the font size
ctx.fillText('انتهت اللعبة', canvas.width / 2 - 80, canvas.height / 2);
      ...
      ctx.fillStyle = 'white';
      ctx.font = '20px Arial';
      ctx.fillText('العب مجددا', canvas.width / 2 - 40, canvas.height / 2 + 45);
    }
  }
